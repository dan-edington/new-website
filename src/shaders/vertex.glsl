...
void main() {
  vUv = uv;
  float t = sin(twopi * uTime/animationSeconds);//sin(twopi*(uTime/animationSeconds));
  vec3 newPos = curlNoise(vec3(
    0.2 * t * (position.x * 0.5) + uTime * 0.1,
    0.25 * t * (position.y * 0.8) + uTime * 0.1,
    0.5 * t * (position.z * 0.2) + uTime * 0.1
  ));

  gl_PointSize = 2.0;
  gl_Position = projectionMatrix * modelViewMatrix * vec4(newPos.xyx, 1.0);
